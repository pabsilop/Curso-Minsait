...
const login = (req, res, next) => {
  ...
  axios
    .get(`http://localhost:3000/users?email=${datosLogin.email}`)
    .then((resp) => {
...
const signup = (req, res = response, next) => {
  ...
  axios
    .get(`http://localhost:3000/users?email=${datosRegistro.email}`)
    .then((resp) => {
      const usuarios = resp.data;
      ...
      if (usuarios.length === 0) {
        ...
        return Promise.resolve({
          data: { msg: "Ya existe un usuario registrado con ese email" },
        });
      }
    })
    .then((resp) => {
